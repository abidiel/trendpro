...
?>
        ...
        <section
            ...
            <?php if (!empty($banner_video_mp4)): ?>
                <video loop autoplay playsinline muted class="html-video" poster="<?php echo $poster_url; ?>" id="dynamic-video">
                    <source id="video-source" type="video/mp4" src="<?php echo esc_url($banner_video_mp4); ?>" />
                </video>
            <?php elseif (!empty($poster_url)): ?>
                <!-- Se não houver vídeo, exibe apenas a imagem de fundo -->
                <div class="background-image" style="background-image: url('<?php echo $poster_url; ?>');"></div>
            <?php endif; ?>
        </section>
        <!-- end page title -->
<?php
...
<section class="bg-dark-gray background-position-center-top overlap-height"
    style="background-image: url('<?php echo get_template_directory_uri(); ?>/assets/images/dotted-pattern.svg')" id="sobre">
